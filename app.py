...
class ECGApp(ctk.CTk):
    ...
    def _export_figures(self) -> None:
        ...
        self._set_status(f"Figures exported — {saved} PNG files  ✓", GREEN)
    ...
    def _build_excel_workbook(self) -> "Workbook":
        ...
        return ExcelExporter.build_workbook(
            ...
            signal_flt   = self._signal_flt,
            signal_raw   = self._signal_raw_norm,
            time         = self._time,
            rpeaks_ok    = self._rpeaks_ok,
            fs           = self._fs,
            filepath     = self._filepath,
            subject      = self.ent_subject.get(),
            sig_quality  = self._sig_quality,
            epoch_df     = self._epoch_df,
        )
    ...
    def _write_excel(self, destination) -> None:
        ...
        ExcelExporter.add_annotations_sheet(wb, self._annotations)
        wb.save(destination)
    ...
    def _export_excel(self) -> None:
        ...
        path = filedialog.asksaveasfilename(
            defaultextension=".xlsx",
            initialfile=f"{self.ent_subject.get()}_ECG_{ts}.xlsx",
        ...
        try:
            ...
            return
        self._set_status("Excel saved", GREEN)
    ...
    def _export_zip(self) -> None:
        ...
        self._set_status("ZIP saved", GREEN)
        messagebox.showinfo("Saved", path)
    ...
    def _export_pdf_report(self) -> None:
        ...
        sub  = self.ent_subject.get().strip() or "subject"
        ts   = datetime.now().strftime("%Y%m%d_%H%M%S")
        path = filedialog.asksaveasfilename(
        ...
        qtc_label = "Mitchell" if self._qtc_formula() == "mitchell" else "Bazett"
        fig.text(0.06, 0.94, f"ECG Report — {sub}",
                 fontsize=15, color=fg, fontweight="bold", va="bottom")
        fig.text(0.06, 0.91,
                 f"Fichier : {self._filepath or '—'}   |   "
        ...
        if self._signal_flt is not None and self._time is not None:
            fs   = self._fs
            dur  = float(self._time[-1])
            t0   = max(0.0, dur / 2 - 5)
            t1   = min(dur, t0 + 10)
            i0, i1 = int(t0 * fs), int(t1 * fs)
            t_seg   = self._time[i0:i1]
            sig_seg = self._signal_flt[i0:i1]
            ...
            if self._rpeaks_ok is not None:
                ...
                rp_w = rp[mask]
                if len(rp_w):
                    ax_ecg.scatter(self._time[rp_w], self._signal_flt[rp_w],
                                   c="red", s=12, zorder=5)
        ...
        self._set_status(f"Rapport PDF sauvegardé — {os.path.basename(path)}", GREEN)
    ...
    def _export_prism(self) -> None:
        ...
        ctx             = EXPERIMENTAL_CONTEXTS.get(self._exp_context)
        beat_corr       = self._results.get("beat_corr")
        beat_peak_times = self._results.get("beat_peak_times")
        arr_events      = getattr(self, "_arrhythmia_events", None) or []
        seg_a           = getattr(self, "_last_seg_a", None)
        seg_b           = getattr(self, "_last_seg_b", None)
        notes           = getattr(self, "_recording_notes", "")
        fs              = float(self._fs) if self._fs else 1000.0

        try:
            n_tables = PrismExporter.build_and_write(
                path              = path,
                results           = self._results,       # type: ignore[arg-type]
                rolling_hrv_df    = getattr(self, "_rolling_hrv_df", None),
                epoch_df          = getattr(self, "_epoch_df", None),
                subject           = sub,
    ...
    def _copy_summary(self) -> None:
        if not self._results:
            ...
            return
        self.clipboard_clear()
        self.clipboard_append(self.txt_sum.get("1.0", "end"))
    ...
    def _save_summary_txt(self) -> None:
        ...
        path = filedialog.asksaveasfilename(
            ...
            initialfile=f"{self.ent_subject.get()}_ECG_summary_{ts}.txt",
        ...
        self._set_status(f"Summary saved: {os.path.basename(path)}", GREEN)
    ...
    def _export_rr_csv(self) -> None:
        ...
        try:
            ...
            self._set_status(f"RR CSV saved → {os.path.basename(path)}", GREEN)
        except Exception as exc:
            messagebox.showerror("Export failed", str(exc))
    ...
    def _export_arrhythmia_pdf(self) -> None:
        ...
        if not self._arrhythmia_events:
            messagebox.showwarning("No arrhythmias", "Run arrhythmia classification first.")
            return
        if self._signal_flt is None or self._time is None or self._fs is None:
        ...
        fs   = self._fs
        rp   = self._rpeaks_ok
        evts = self._arrhythmia_events
        WIN  = 6.0   # seconds per strip

        self._set_status("Building arrhythmia PDF…", ORANGE)
        try:
